...
#===========================================================================
setwd("~/git/residual_reversal_201903")
...
ff4f.tw = read.table("./data/FF4F_1990_2017_m.txt", stringsAsFactors = FALSE, fileEncoding = 'UTF-8')
...
#===========================================================================================
# import stock index
twse = read.table("./data/TWSE_index_1990_2017_m_close.txt", fileEncoding = 'UTF-8')
twse = twse[-1, c(-1, -2, -3)]
...
#models.tw<-list()
#data.tw <- new.env()
#將沒有資料的股票刪除(即全部是NA資料者)=>輸出為price.tw.f;
...
coeff.meds.es.up.df
write.csv(coeff.meds.es.up.df, file="~/git/residual_reversal_201903/output/tables/coeff_meds_es_up_10Q.csv")
#------------------------------------------------------------
# Replicate Blitz Table 1 using Eq(15) but we use Q1-Q10 
...
write.csv(coeff.meds.ret.up.df, file="~/git/residual_reversal_201903/output/tables/coeff_meds_ret_up_10Q.csv")
#----------------------------------------------------
...
for (i in 1:3){
  ...
  factor.ret = lag(es.qi.ret.factors[,i+1],1)
  #head(factor.ret)
